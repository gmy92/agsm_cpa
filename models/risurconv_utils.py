...
def index_points(points, idx):
    ...
    # Ensure idx is on the same device as points
...
def compute_LRA_one(group_xyz, weighting=False):
    ...
    # Add checks for NaN values and fix them
    M[torch.isnan(M)] = 0.0
    M[torch.isinf(M)] = 0.0
    
    # Add a small epsilon to the diagonal to ensure numerical stability
    epsilon = 1e-6
    M = M + epsilon * torch.eye(M.shape[-1], device=M.device).unsqueeze(0).unsqueeze(0)
    
    try:
        eigen_values, vec = torch.linalg.eigh(M, UPLO='U')
    except Exception as e:
        # If eigh still fails, use a fallback method
        print(f"Warning: eigh failed with error: {e}. Using fallback method.")
        # Create a dummy LRA vector
        LRA = torch.zeros(B, S, C, device=group_xyz.device)
        LRA[:, :, 2] = 1.0  # Set z-component to 1 as a fallback
        return LRA
    
    # Check for NaN in eigenvalues and eigenvectors
    eigen_values[torch.isnan(eigen_values)] = 0.0
    ...
    LRA_length = torch.clamp(LRA_length, min=1e-8)
    LRA = LRA / LRA_length
    
    # Check for NaN in final result
    LRA[torch.isnan(LRA)] = 0.0
...
def knn_point(nsample, xyz, new_xyz):
    ...
    # Ensure nsample does not exceed the number of available points
    B, N, C = xyz.shape
    _, S, _ = new_xyz.shape
...
def sample(npoint, xyz, norm=None, sampling='fps'):
    B, N, C = xyz.shape
    xyz = xyz.contiguous()
    new_norm = None  # Initialize new_norm to None
    if sampling=='fps':
...
def order_index(xyz, new_xyz, new_norm, idx):
    epsilon=1e-7
    B, S, C_xyz = new_xyz.shape  # C for xyz coordinates
    _, _, C_norm = new_norm.shape  # C for normal vectors (might be different)
    nsample = idx.shape[2]
    grouped_xyz = index_points(xyz, idx)
    grouped_xyz_local = grouped_xyz - new_xyz.view(B, S, 1, C_xyz)  # centered

    # project and order
    if new_norm is not None:
        # Ensure tensors are contiguous
        new_norm = new_norm.contiguous()
        grouped_xyz_local = grouped_xyz_local.contiguous()
        
        # For matrix multiplication, we need to reshape grouped_xyz_local to [B*S, nsample, C_xyz]
        grouped_xyz_local_reshaped = grouped_xyz_local.reshape(B*S, nsample, C_xyz)
        
        # Reshape new_norm for proper matrix multiplication
        new_norm_expanded = new_norm.unsqueeze(2)  # [B, S, 1, C_norm]
        
        # Handle the tensor reshaping correctly to ensure matrix multiplication works
        # We need to reshape new_norm_expanded to [B*S, 1, C_norm]
        total_elements = new_norm_expanded.numel()
        expected_elements = B * S * 1 * C_norm
        
        if total_elements == expected_elements:
            new_norm_expanded_reshaped = new_norm_expanded.reshape(B*S, 1, C_norm)
        else:
            # If the shapes don't match, we need to handle this gracefully
            # Let's try to find the correct dimensions
            if C_norm > 0 and total_elements % C_norm == 0:
                correct_first_dim = total_elements // C_norm
                new_norm_expanded_reshaped = new_norm_expanded.reshape(correct_first_dim, 1, C_norm)
            else:
                # Fallback: flatten and reshape
                new_norm_flat = new_norm_expanded.view(-1)
                usable_elements = (new_norm_flat.shape[0] // C_norm) * C_norm if C_norm > 0 else new_norm_flat.shape[0]
                correct_first_dim = usable_elements // C_norm if C_norm > 0 else usable_elements
                new_norm_expanded_reshaped = new_norm_flat[:usable_elements].reshape(correct_first_dim, 1, C_norm if C_norm > 0 else 1)
        
        # Perform matrix multiplication only if dimensions match
        if C_xyz == C_norm:
            # Transpose to get [correct_first_dim, C_norm, 1] for matmul with [B*S, nsample, C_xyz]
            norm_transposed = new_norm_expanded_reshaped.transpose(-1, -2)
            dist_plane_reshaped = torch.matmul(grouped_xyz_local_reshaped, norm_transposed)
            
            # Reshape back to [B, S, nsample, 1]
            dist_plane = dist_plane_reshaped.reshape(B, S, nsample, 1)
            proj_xyz = grouped_xyz_local - dist_plane * new_norm_expanded
        else:
            # If dimensions don't match, we need to handle this case differently
            # Use only the first C_xyz dimensions of new_norm for projection
            if C_norm >= C_xyz:
                # Use only the first C_xyz dimensions of new_norm
                new_norm_reduced = new_norm[:, :, :C_xyz]  # [B, S, C_xyz]
                new_norm_expanded_reduced = new_norm_reduced.unsqueeze(2)  # [B, S, 1, C_xyz]
                new_norm_expanded_reshaped_reduced = new_norm_expanded_reduced.reshape(B*S, 1, C_xyz)
                
                # Transpose to get [B*S, C_xyz, 1] for matmul with [B*S, nsample, C_xyz]
                norm_transposed = new_norm_expanded_reshaped_reduced.transpose(-1, -2)
                dist_plane_reshaped = torch.matmul(grouped_xyz_local_reshaped, norm_transposed)
                
                # Reshape back to [B, S, nsample, 1]
                dist_plane = dist_plane_reshaped.reshape(B, S, nsample, 1)
                proj_xyz = grouped_xyz_local - dist_plane * new_norm_expanded_reduced
            else:
                # Pad new_norm to match C_xyz dimensions
                padding = torch.zeros(B, S, C_xyz - C_norm, device=new_norm.device)
                new_norm_padded = torch.cat([new_norm, padding], dim=-1)  # [B, S, C_xyz]
                new_norm_expanded_padded = new_norm_padded.unsqueeze(2)  # [B, S, 1, C_xyz]
                new_norm_expanded_reshaped_padded = new_norm_expanded_padded.reshape(B*S, 1, C_xyz)
                
                # Transpose to get [B*S, C_xyz, 1] for matmul with [B*S, nsample, C_xyz]
                norm_transposed = new_norm_expanded_reshaped_padded.transpose(-1, -2)
                dist_plane_reshaped = torch.matmul(grouped_xyz_local_reshaped, norm_transposed)
                
                # Reshape back to [B, S, nsample, 1]
                dist_plane = dist_plane_reshaped.reshape(B, S, nsample, 1)
                proj_xyz = grouped_xyz_local - dist_plane * new_norm_expanded_padded
    else:
        # When new_norm is None, use a dummy projection
        proj_xyz = grouped_xyz_local
    
    proj_xyz_length = torch.norm(proj_xyz, dim=-1, keepdim=True)
    projected_xyz_unit = proj_xyz / (proj_xyz_length + epsilon)
    ...
    # Fix the shape issue by properly handling the dimensions
    # length_max_idx has shape [B, S, 1] (after argmax with keepdim), we need [B, S, 1, C_xyz] for gather
    length_max_idx_expanded = length_max_idx.unsqueeze(-1).expand(B, S, 1, C_xyz)
    vec_ref = projected_xyz_unit.gather(2, length_max_idx_expanded) # corresponds to the largest length
    
    # Fix the matmul operation to match the correct dimensions
    dots = torch.matmul(projected_xyz_unit, vec_ref.view(B, S, C_xyz, 1))
    
    # Fix the cross product operation - only apply cross product when C_xyz=3 (3D vectors) and new_norm is not None
    if C_xyz == 3 and new_norm is not None:
        # Ensure tensors are contiguous
        new_norm = new_norm.contiguous()
        sign = torch.cross(projected_xyz_unit, vec_ref.view(B, S, 1, C_xyz).expand_as(projected_xyz_unit), dim=-1)
        # Reshape for proper matrix multiplication
        sign_reshaped = sign.reshape(B*S, nsample, C_xyz)
        
        # Handle new_norm reshaping with the same approach as above
        if C_norm >= C_xyz:
            # Use only the first C_xyz dimensions of new_norm
            new_norm_reduced = new_norm[:, :, :C_xyz]  # [B, S, C_xyz]
            new_norm_expanded_reduced = new_norm_reduced.unsqueeze(2)  # [B, S, 1, C_xyz]
            new_norm_reshaped = new_norm_expanded_reduced.reshape(B*S, 1, C_xyz)
            
            # Transpose to get [B*S, C_xyz, 1] for matmul with [B*S, nsample, C_xyz]
            norm_transposed = new_norm_reshaped.transpose(-1, -2)
            sign_result_reshaped = torch.matmul(sign_reshaped, norm_transposed)
            
            # Reshape back to [B, S, nsample, 1]
            sign_result = sign_result_reshaped.reshape(B, S, nsample, 1)
            sign = torch.sign(sign_result)
        else:
            # For cases where dimensions don't match, create a dummy sign tensor
            sign = torch.ones_like(dots)
    else:
        # For cases where C_xyz != 3 or new_norm is None, create a dummy sign tensor
        sign = torch.ones_like(dots)
    
    # Fix the sign assignment
    sign_first = sign.clone()
    sign_first[:, :, 0, 0] = 1.  # the first is the vec_ref itself, should be 1.
    dots = sign_first*dots - (1-sign_first)
...
def RISP_features(xyz, norm, new_xyz, new_norm, idx, group_all=False):
    ...
    # Handle case when new_norm is None
    if new_norm is not None:
        # Ensure new_norm is contiguous
        new_norm = new_norm.contiguous()
        dots_sorted, idx_ordered = order_index(xyz, new_xyz, new_norm, idx)
    else:
        # Create a dummy norm for order_index when norm is not available
        dummy_norm = torch.zeros_like(new_xyz)
        dots_sorted, idx_ordered = order_index(xyz, new_xyz, dummy_norm, idx)
    
    grouped_center = index_points(xyz, idx_ordered)  # [B, npoint, nsample, C]
    if norm is not None:
        xi_norm = index_points(norm, idx_ordered)             # xi norm
    else:
        xi_norm = torch.zeros_like(grouped_center)            # dummy norm when not available
    ...
    if new_norm is not None:
        ...
        p_norm = torch.zeros_like(p_point)  # dummy norm when not available
...
def sample_and_group(npoint, radius, nsample, xyz, norm):
    ...
    Input:
        ...
        norm: input points norm data, [B, N, 3]
    Return:
