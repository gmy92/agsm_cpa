...
class ScanObjectNN(Dataset):
    def __init__(self, root, args, split='train'):
        ...
        if self.uniform:
            ...
            # Fix the slicing to properly select the first num_points points
            self.points = self.points[:, :self.num_points, :]
        
        # Convert to tensor first to avoid double conversion
        if not isinstance(self.points, torch.Tensor):
            original_points = torch.from_numpy(self.points).float()
        else:
            original_points = self.points
        
        # compute the normal vector based on LRA
        norm = torch.zeros_like(original_points)
        for i in range(original_points.shape[0]):
            norm_i = compute_LRA(original_points[i,:,:].unsqueeze(0), True, nsample = 32)
            norm[i,:,:] = norm_i

        self.points = torch.cat([original_points, norm], dim=-1)
